...
#TCP server and client that leave too much data waiting
...
def server(host, port, bytecount):
	...
	while True:
		...
		while True:
			...
			sc.sendall(output) # send it back upppercase
			n += len(data)
...
def client(host, port, bytecount):
	...
	print('Sending', bytecount, 'bytes of data, in chunk of 16 bytes')
