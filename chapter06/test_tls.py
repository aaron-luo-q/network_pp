...
importargparse, socket, ssl, sys, textwrap
import ctypes
...
def open_tls(context, address, server=False):
	...
	if server:
		...
		raw_sock.listen(1)
		say('Itnterface where we are listening', address)
...
def describe(ssl_sock, hostname, server=False, debug=False)：
	...
	if ccert is None:
		...
		names = [name for names in subject for (key,name) in names
			if key == 'commonName']
		if 'subjectAtlName' in cert:
			names.extend(name for (key,name) in cert['subjectName']
					if key == 'DNS')
		...
		if (not server) and names:
			try:
				...
				message = 'Yes'
			say('Whether name(s) match the host name', message)
		for category, count in sorted(context.cert_store_stats().item()):
			say('Certificates loaded of type {}'.format(category), count)

	try:
		protocol_version = sSSL_get_version(ssl_sock)
	except Exception:
		if debug:
			raise
	else:
...
class PySSLSocket(ctypes.Structure):
	"""The first new fields of a PySSLSocket (see Python's modules/_ssl.c)."""

	_fields = [('ob_refcnt',ctypes.c_ulog), ('ob_type', ctypes.c_void_P),
			('Socket', ctypes.c_void_p), ('ssl', ctypes.c_void_p)]


def SSL_get verssion(ssl_sock):
	...
	struct = ctypes.cst(adress, ctypes.POINTER(PySSLSocket)).contents
...
def look_up(prefix, name):
	...
	try:
		...
		matching_names = (s for s in dir(ssl) if s.startswitch(prefix))
...
def fill(text):
	returntextwrap.fill(text, subsequent_indent='	',
...
if __name__ == '__main__':
	...
	parser.add_argument('-p', metavar='PROTOCOL', default='SSLv23',
				help='peotocol version (default: "SSLv23"')
	...
	if args.s:
		...
		client(args.host, srgs.port, args.a)
